...
class HyperNetwork(nn.Module):
    ...
    def __init__(self, in_out_dim, hidden_dim, width):
        ...
        self.fc3 = nn.Linear(hidden_dim, 3 * blocksize + width) #3 * blocksize + width
    ...
    def forward(self, t, z): #N=batch_size, d=2, width = 64, hidden_dim = 32, blocksize = 128
        ...
        Z = torch.unsqueeze(z, 0).repeat(self.width, 1, 1) # [width,N,d]  64,32,2
        # compute function output
        h = torch.tanh(Z@W+B) # [width,N,1]
        f = (h@U).mean(0) # [N,d] - mean over hidden units
        return f
